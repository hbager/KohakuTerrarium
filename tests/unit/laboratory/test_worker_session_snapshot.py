...
@pytest.mark.asyncio
async def test_close_graph_flushes_final_runtime_group_meta(tmp_path):
    root_cfg = _write_dummy_agent_cfg(tmp_path, "root")
    worker_cfg = _write_dummy_agent_cfg(tmp_path, "worker")
    node = _RecordingNode()
    engine = Terrarium()
    attacher = WorkerSessionAttacher(
        engine, node, session_dir=tmp_path / "worker-sessions"
    ...
    try:
        root = await engine.add_creature(str(root_cfg), io="none", strict=False)
        attacher.attach(root.creature_id)
        ...
        await attacher.close_graph(root.graph_id)

        meta_messages = [
            kwargs["body"]["meta"]
            for _args, kwargs in node.notified
            if kwargs.get("type") == "meta"
        ]
        assert len(meta_messages) >= 2
        final_meta = meta_messages[-1]
        assert final_meta["status"] == "paused"
        assert {item["name"] for item in final_meta["runtime_creatures"]} == {
            "root",
            "worker",
        }
        assert engine._session_stores == {}
        assert attacher._graph_tees == {}
        assert attacher._graph_refs == {}
    finally:
        attacher.close_all()
        await engine.shutdown()
