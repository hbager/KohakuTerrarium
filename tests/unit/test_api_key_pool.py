...
def test_anthropic_provider_uses_string_not_pool_for_sdk_client():
    provider = AnthropicProvider(api_key=KeyPool(["k1", "k2"]), model="claude-test")

    assert provider._api_key == "k1"
    assert isinstance(provider._api_key, str)
    assert provider._api_key_pool.keys == ("k1", "k2")


def test_anthropic_provider_applies_rotating_x_api_key_header():
    provider = AnthropicProvider(api_key=KeyPool(["k1", "k2"]), model="claude-test")
    first: dict = {}
    second: dict = {}
    ...
    assert first["extra_headers"]["X-Api-Key"] == "k1"
    assert second["extra_headers"]["X-Api-Key"] == "k2"


def test_anthropic_provider_bearer_rotates_authorization_header():
    provider = AnthropicProvider(
        api_key=KeyPool(["k1", "k2"]),
        model="claude-test",
        auth_as_bearer=True,
    )
    first: dict = {}
...
@pytest.mark.asyncio
async def test_anthropic_provider_user_error_failover_stops_after_five_keys():
    provider = AnthropicProvider(
        api_key=KeyPool(["k1", "k2", "k3", "k4", "k5", "k6"]),
        model="claude-test",
    )
    ...
    async def always_unauthorized(messages):
        create_kwargs: dict = {}
        provider._apply_request_api_key(create_kwargs)
        seen_keys.append(create_kwargs["extra_headers"]["X-Api-Key"])
        raise _StatusError(401)
    ...
    assert seen_keys == ["k1", "k2", "k3", "k4", "k5"]
...
def test_studio_set_key_normalizes_comma_separated_pool(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: {"openai": ("openai", "OPENAI_API_KEY")},
    )
    monkeypatch.setattr(
        studio_api_keys,
        "save_api_key",
