...
async def session_channel_observer(
    ...
    # Validate cluster-wide existence first so a missing session produces an
    # explicit error instead of an indefinitely idle subscription.
            ...
            await websocket.send_json(
                {
                    ...
                    # Display names are not unique, so clients also need the
                    # stable creature handle to identify the sender.
