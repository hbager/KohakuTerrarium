  ...
  <div class="h-full flex flex-col bg-warm-50 dark:bg-warm-900 overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto px-3 py-2 text-xs">
      <template v-if="isTerrarium">
        <div class="mb-3">
          <div class="text-[10px] uppercase tracking-wider text-warm-400 font-medium mb-1">Creatures</div>
          <div class="flex flex-col gap-1">
            <div v-for="c in creatures" :key="c.name" class="flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer transition-colors hover:bg-warm-100 dark:hover:bg-warm-800" :class="activeTab === c.name ? 'bg-iolite/10' : ''" @click="onOpenTab(c.name)">
        ...
        <div v-if="channels.length">
          <div class="text-[10px] uppercase tracking-wider text-warm-400 font-medium mb-1">Channels</div>
          <div class="flex flex-col gap-1">
            <div v-for="ch in channels" :key="ch.name" class="flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer transition-colors hover:bg-warm-100 dark:hover:bg-warm-800" :class="activeTab === 'ch:' + ch.name ? 'bg-taaffeite/10' : ''" @click="onOpenTab('ch:' + ch.name)">
      ...
      <template v-else>
        <div class="text-warm-400 py-6 text-center text-[11px]">Not a terrarium — single creature runs standalone.</div>
      </template>
    </div>
  </div>
</template>
