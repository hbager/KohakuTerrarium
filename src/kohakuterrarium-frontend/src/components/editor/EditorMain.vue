<template>
  <div class="h-full flex flex-col">
    ...
    <div v-if="editor.openFilePaths.length > 0" class="flex items-center gap-0.5 px-2 h-7 border-b border-warm-200 dark:border-warm-700 overflow-x-auto shrink-0">
      ...
      <button v-if="isMarkdown" class="flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] transition-colors shrink-0" :class="useVditor ? 'bg-iolite/15 text-iolite' : 'text-warm-400 hover:text-warm-600 dark:hover:text-warm-300'" :title="useVditor ? 'Switch to code editor' : 'Switch to rich markdown editor'" @click="useVditor = !useVditor">
