<template>
  <div>
    <!-- Directory -->
    <div v-if="node.type === 'directory'" class="flex items-center gap-1 px-2 py-1 cursor-pointer select-none hover:bg-warm-100 dark:hover:bg-warm-800 transition-colors" :style="{ paddingLeft: depth * 14 + 8 + 'px' }" @click="expanded = !expanded">
      <div class="text-warm-400 text-[10px] w-3 shrink-0 transition-transform" :class="expanded ? 'i-carbon-chevron-down' : 'i-carbon-chevron-right'" />
