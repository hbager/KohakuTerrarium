<template>
  <div class="fixed bottom-4 right-4 flex flex-col gap-2 z-[9999] pointer-events-none">
    <div v-for="t in notifications.toasts.slice(-6)" :key="t.id" class="min-w-64 max-w-96 rounded-lg border px-3 py-2 text-xs shadow-lg backdrop-blur bg-white/95 dark:bg-warm-900/95 pointer-events-auto flex items-start gap-2" :class="levelBorder(t.level)">
      ...
      <button class="text-warm-400 hover:text-warm-600 dark:hover:text-warm-300 shrink-0" title="Dismiss" @click="notifications.dismiss(t.id)">
