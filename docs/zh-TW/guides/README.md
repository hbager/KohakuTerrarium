...
- [Docker 部署](deployment-docker.md)：三種 compose 模式（AIO、host + worker、分散式），均使用 GHCR 官方映像。
- [systemd 部署](deployment-systemd.md)：透過 `kt service install` 安裝為強化後的 systemd 服務。
- [反向代理部署](deployment-reverse-proxy.md)：TLS 終止的 nginx 與 Cloudflare Tunnel 設定。
