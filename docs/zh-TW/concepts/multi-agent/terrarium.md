...
引擎是行程層級的 host。一個行程一個引擎。引擎裡可以同時存在任意多張 graph（你的團隊、臨時拉起來快速對話的獨立生物、沒有同儕的監控生物），每張都是獨立的 connected component。拓樸不是凍結的；channel 可以在 runtime 跨 graph 拉起來（造成 graph 合併），也可以拆掉（可能讓 graph 分裂回去）。
