...
- [会话持久化](session-persistence.md) — `.kohakutr` 文件格式、每个 Creature 存什么、恢复时怎么重建对话状态。
- [图与 session](graph-and-sessions.md) — terrarium 引擎如何计算连通分量、在热插拔呼叫下变更拓扑，并在自动合并 / 自动分裂中保持 session store 的一致。
